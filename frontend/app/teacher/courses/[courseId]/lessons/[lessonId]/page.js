  ...
  async function loadStudents() {
    try {
      const response = await fetch(
        `http://localhost:3000/lessons/${lessonId}/students`
      );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#f5f0eb]">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-[#6f311c] font-medium">Loading lesson progress...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#f5f0eb]">
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8">
        ...
        {/* Progress bar */}
        <div className="mt-6 sm:mt-8 bg-white rounded-2xl shadow-md border border-[#eee3d6] p-6">
          <div className="flex justify-between items-center mb-2">
            <p className="text-sm font-medium text-[#3b130d]">Overall progress</p>
            <p className="text-sm font-semibold text-[#6f311c]">{completionRate}%</p>
          </div>
          <div className="w-full h-2.5 bg-[#f0ded0] rounded-full overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-[#6f311c] to-[#8b4513] rounded-full transition-all duration-500"
          ...
          {students.length === 0 ? (
            <div className="flex flex-col items-center justify-center text-center py-16 px-6 bg-white/60 border border-dashed border-[#c9a68a] rounded-2xl">
              ...
              <p className="text-gray-500 mt-2 max-w-sm">
                Once students start this lesson, their progress will show up here.
              </p>
            </div>
          ) : (
